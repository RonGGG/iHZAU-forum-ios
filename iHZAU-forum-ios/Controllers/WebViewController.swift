...
class WebViewController: VisitableViewController {
    ...
    fileprivate lazy var router: Router = {
        ...
        router.bind("/posts") { [weak self] (req) in
            self?.pageTitle = "posts".localized
        }
        router.bind("/sites") { [weak self] (req) in
            self?.pageTitle = "cool site".localized
        }
    ...
    convenience init(path: String) {
        self.init()
        currentPath = path
        visitableURL = urlWithPath(path) //加上ROOT前缀
    }
...
extension WebViewController {
    ...
    fileprivate func addObserver() {
        ...
        NotificationCenter.default.addObserver(forName: NSNotification.Name.userSignout, object: nil, queue: nil)
        { [weak self] (notification) in guard let `self` = self else {
                return
            }
            self.clearSession()
