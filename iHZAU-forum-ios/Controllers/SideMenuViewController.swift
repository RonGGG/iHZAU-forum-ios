        ...
        if let user = OAuth2.shared.currentUser, OAuth2.shared.isLogined {
            ...
            datas.append([
                ItemData(
                    ...
                    image: UIImage(named: "posts")!.withRenderingMode(.alwaysTemplate),
                    imageColor: PRIMARY_COLOR,
                    actionURL: URL(string: "\(ROOT_URL)/posts")!
                ),
                ItemData(
                    name: "cool site".localized,
                    image: UIImage(named: "sites")!.withRenderingMode(.alwaysTemplate),
                    imageColor: PRIMARY_COLOR,
                    actionURL: URL(string: "\(ROOT_URL)/sites")!
                )
            ])
            datas.append([
                ItemData(
...
//        print(URL(string: ROOT_URL)!.host!)
        if host != URL(string: ROOT_URL)!.host! {
            ...
            dismiss(animated: true, completion: {
                if let host = url.host, host != URL(string: ROOT_URL)!.host! {
                    ...
                    TurbolinksSessionLib.shared.action(.Advance, path: url.path)// url.path:/posts
                }
            })
        }
    }
